...
avg_rating_non_free=float(sum(non_free_apps_ratings))/len(non_free_apps_ratings)
...
for row in apps_data[1:]:
    ...
    price=float(row[4])
    if (price>9)  :
        price_9.append(rating)
        n_apps_more_9+=1
    if (price<=9):
        n_apps_less_9+=1
avg_rating=sum(price_9)/len(price_9)
